...
Utilities.prototype = {
  ...
  checkIfExists: function (tableName, fieldName, stringToCheck, getResponse) {
    ...
    async.each(tableArr, function (table, callback) {
      _O.checkIfExistsHelper(table, fieldName, stringToCheck,
        function (result, result2) {
          // console.log(result,result2);
          checkSum += result;
          data = (result2 !== undefined && !_O._u.isEmpty(result2) &&
            result == 1) ? result2 : data;
          table_found = (result2 !== undefined && !_O._u.isEmpty(result2) &&
            result == 1) ? table : table_found;
          callback();
        });
    }, function (err, result) {
      if (err)
  ...
  checkIfExistsHelper: function (
    tableName, fieldName, stringToCheck, getResponse) {
    //
    ...
    pool.getConnection(function (err, connection) {
      if (err) {
        ...
        var queryString = 'SELECT * FROM `' + tableName + '` WHERE (`' + fieldName +
          '`= ? ' + extraWhere + ') AND `publish` = 1 LIMIT 1';
        let str = connection.query(queryString, values, function (err, result) {
          connection.release();
          ...
          console.log(" extra where ", extraWhere, " queer query : ", str.sql);
        });
      }
    });
  },
  ...
  getMessage: function (lang, input, data, mtype) {
    ...
    if (input instanceof Object) {
      ...
      if (input == 1) {
        ...
        type = 'error';
        message = 'Not enough money in your Kopo Account. Your Kopo balance is ' + data;
        title = 'Failed Balance';
      } else if (input == 10) {
        type = 'error';
        message = data;
        title = 'Mpesa Error';
  ...
  /**
   * Reads The Constants from a file
   * Compare given value with constant
   * Evaluates to either true/ False
   */
  mobiCheck: function (security, type) {
    var sys_config = JSON.parse(fs.readFileSync('config/sys_config.json'));
    let key = '';
    let result;
    switch (type) {
      case 'mobi':
        key = sys_config.constants.mobi;
        key === security ? result = true : result = false;
        break;
      default:
        result = false;
    }
   ...
   * @returns {string}
   */
  dynamicDataAccessWhereClause: function (
    req, tableName, orgIdDbName, userIdDbName, groupColumn, groupTable,
    getResponse) {
    ...
    let linked_user_id = (req.query.linked_user_id)
      ? req.query.linked_user_id
      : req.session.LINKED_USER_ID;
    //
    ...
    //
    try {
      _O.dynamicDataAccessWhereClauseGroupHelper(req, where, tableName,
        orgIdDbName, 'party',
        function (resultOrg, resultArrayOrg, resultOrgCheck) {
          _O.dynamicDataAccessWhereClauseGroupHelper(req, where, tableName,
            groupColumn, groupTable,
            function (result, resultArray, resultCheck) {
              // console.log(resultOrg,resultOrgCheck,resultArrayOrg,result,resultCheck, resultArray);
              let final_clause = (resultOrgCheck == true &&
                !_O._u.isEmpty(resultArrayOrg)) ? resultOrg : result;
              final_clause = ('table_primary_key' in req) ? '(' +
                final_clause + ' OR ' + tableName + '.' +
                req.table_primary_key + ' = ' + user_id + ' OR ' +
                tableName + '.' + req.table_primary_key + ' = ' +
                linked_user_id + ')' : final_clause;
  ...
  dynamicDataAccessWhereClauseGroupHelper: function (
    req, whereClause, tableName, groupColumn, groupTable, getResponse) {
    let _O = this;
    //
    //
    let user_groups = (req.query.user_groups)
      ? req.query.user_groups
      : req.session.USER_GROUPS;
    ...
    let linked_user_id = (req.query.linked_user_id)
      ? req.query.linked_user_id
      : req.session.LINKED_USER_ID;
    let linked_user_table_name = (req.query.linked_user_id_module)
      ? req.query.linked_user_id_module
      : req.session.LINKED_USER_TABLE_NAME;
    //
    // console.log(user_type,linked_user_table_name,req.session);
    //
    if (user_type == '1' && groupColumn != '') {
      let needed_ids = '';
      _O.dynamicDataAccessWhereClauseGroupHelperHelper(
        _O.implodeAssoc(_O.tryParseJSON(user_groups), 'id'), groupTable,
        function (result) {
          result.forEach(function (entry) {
            needed_ids += _O.implodeAssoc(_O.tryParseJSON(entry['ITEMS']),
              'id') + ',';
          });

          whereClause = 'FIND_IN_SET(`' + tableName + '`.`' + groupColumn +
            '`,\'' + needed_ids.substring(0, needed_ids.length - 1) + '\')';
  ...
  dynamicDataAccessWhereClauseGroupHelperHelper: function (
    ids, mdtable, getResponse) {
    ...
    this.dbo.getConnection(function (err, connection) {
      if (err) {
        ...
        connection.query(
          'SELECT * FROM `user_groups` WHERE FIND_IN_SET(`ID`,?) AND `MODULE_TABLE` = ?',
  ...
  getLanguageVariables: function (req, moduleName, emptyObjectIfFail = false) {
    ...
    var data = {};
    var folderName = '';
    if (cookieObj && 'LANG' in cookieObj) {
    ...
    try {
      data = JSON.parse(fs.readFileSync(
        'config/lang/' + folderName + '/' + moduleName + '.json'));
    } catch (err) {
  ...
  getDBNameSetByDate: function (date) {
    ...
    var oncheck_date = (moment(date, 'YYYY-MM-DD HH:mm:ss', true).isValid() ==
      true || moment(date, 'YYYY-MM-DD', true).isValid() == true) ? moment(
  ...
  getLocationFromCoordinates: function (lat, lon, getResponse) {
    ...
    let url_path_params = '?format=json&lat=' + lat + '&lon=' + lon +
      '&zoom=16';
  ...
  sanitizePhoneNumber: function (country_code, phone_number) {
    let res = phone_number;
    try {
      if (phone_number.length === 10 && phone_number.substr(0, 1) == '0') {
        if (country_code != '' || country_code == '' || country_code ==
          undefined) {
          country_code = "254";
        }
        res = country_code + phone_number.substr(1, 10);
      } else if (phone_number.length === 9 && phone_number.substr(0, 1) !=
        '0' && (country_code != '' || country_code == '' || country_code ==
          undefined)) {
        if (country_code == '' || country_code == undefined) {
          country_code = '254';
        }
        res = country_code + phone_number;
      } else if ((country_code == phone_number.substr(0, 3) &&
        phone_number.length === 12) ||
        (phone_number.length === 12 && country_code == '')) {
        if (country_code != phone_number.substr(0, 3) && country_code != '') {
          res = country_code + phone_number.substr(3, 12);
        } else {
          res = phone_number;
        }
      } else if (phone_number.length === 13 && phone_number.substr(0, 1) == '+') {
        res = phone_number.substr(1,);
      }
  ...
  randomness: function (length = 256) {
    return crypto.randomBytes(Math.ceil(length / 2)).
      toString('hex') /** convert to hexadecimal format */
      .
      slice(0, length);
  },
  formatResponse: function (formatArr, resultArr) {
    if (formatArr == undefined) return resultArr;
    let len = formatArr.length;
    if (len === 0) return resultArr;
    let resObj = {}, i = 0;
    while (i < len) {
      let el = formatArr[i], result = Object.fromEntries(
        Object.entries(resultArr).
          filter(([key, value]) => key === el));
      resObj[el] = result[el];
      i++;
    }
    return resObj;
  },
  gateKeeper: function (req, res) {
    if (req.session.USER_OBJ === undefined) {
      res.redirect("/");
      throw new Error("Something went badly wrong!");
    }
  },
  callback: function (payload, counter, req, res, len, mobi = false) {
    let _O = this;
    let stat = counter == len;
    console.log(payload, counter, stat, len);
    if (payload == 1 && stat) {
      user.id = req.body.userId;
      // if(mobi){
      //   user.getById(function(result) {
      //     res.send(_O.getMessage('', 1,
      //         _O.formatResponse(req.body.resultObject, result[0])));
      //   });
      // }else{
      res.status(200).send(_O.getMessage("", payload, ""));
      // }
      console.log("here");
    } else {
      if (stat) {
        res.status(400).send(_O.getMessage('', 8, ''));
      }
    }
  },
  checkIfExistsHelperInBetween: function (tableName, fieldName1, fieldName2, stringToCheck, getResponse, data = '') {
    //
    var extraWhere = this.extraWhere;
    //
    pool.getConnection(function (err, connection) {
      if (err) {
        ...
        var query = "SELECT * FROM `" + tableName + "` WHERE '" + stringToCheck + "' BETWEEN `" + fieldName1 + "` AND  `" + fieldName2 + "`" + extraWhere + " AND `publish` = 1 LIMIT 1";
        connection.query(query, function (err, result) {
          ...
          if (err) {
            ...
            if (result instanceof Array && result.length === 0) {
              ...
              data == '' ? getResponse(1, result) : getResponse(result, 1);
            }
          }
        });
      }
  ...
  isFileImage: function (fileExt) {
    return (fileExt === '.tif' || fileExt === '.tiff' || fileExt === '.bmp' || fileExt === ".jpg" || fileExt === ".jpeg" || fileExt === ".gif" || fileExt === ".png");
  },
